...
const assertWalletsUpdatedMultiTrade = (o, dataset, feeMultiplier) => {
  ...
  const subOrders = statuses.filter(s => s.indexOf('@') !== -1).map(s => {
    ...
    const amount = +values[1].substring(0, values[1].length - 1)
...
const assertWalletsUpdated = (o, dataset, feeMultiplier, noRecursion = false) => {
  ...
  // The verbose errors are very useful for debugging..
  if (Object.values(found).filter(v => v !== null).length !== 4) {
    for (let i = 0; i < qWalletUpdates.length; i += 1) {
      const wu = qWalletUpdates[i]

      const orderDelta = Math.abs(wu.delta - orderQChange)
      const feeDelta = Math.abs(wu.delta - feeQChange)

      debugV(`Q wu ${wu.type}-${wu.currency} balance ${wu.balance} delta ${wu.delta} order delta ${orderDelta} fee delta ${feeDelta}`)
    }

    for (let i = 0; i < bWalletUpdates.length; i += 1) {
      const wu = bWalletUpdates[i]

      const orderDelta = Math.abs(wu.delta - orderBChange)
      const feeDelta = Math.abs(wu.delta - feeBChange)

      debugV(`B wu ${wu.type}-${wu.currency} balance ${wu.balance} delta ${wu.delta} order delta ${orderDelta} fee delta ${feeDelta}`)
    }
