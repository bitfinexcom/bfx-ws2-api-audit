...
module.exports = ({
  ...
  return {
    ...
    exec: ({ dataM, dataT, restM, wsM, wsT, refreshSnapshots }) => {
      ...
      const oSellM = poUtils.genLimitSell({ price: pMidPrice * 1.0002 })
      const oBuyM = poUtils.genLimitBuy({ price: pMidPrice * 1.0001 })
      ...
      return restM.exchangeRate(primaryQuote, virtualQuote).then(rate => {
        ...
        // submit virtual taker MARKET orders
          ...
          dataM.updateOrder(oSellM)
          // dataM.updateOrder(voBuyM)
          // dataM.updateOrder(voSellM)

          assertFullyFilled(voBuyT) // taker orders
          assertFullyFilled(voSellT)
          assertWalletsUpdated(voBuyT, dataT, T_FEE)
          assertWalletsUpdated(voSellT, dataT, T_FEE)

          assertFullyFilled(voBuyM) // maker orders
          assertFullyFilled(voSellM)
          ...
          assertWalletsUpdated(voBuyM, dataM, M_FEE)
          assertWalletsUpdated(voSellM, dataM, M_FEE)
        })
      })
    }
  }
