...
module.exports = ({ symbol, amount, initialLast, dataDelay }) => {
  ...
  return {
    id: 'limit_as_market_beyond_ticker',
    ...
    exec: ({ dataM, dataT, wsM, wsT, refreshSnapshots }) => {
      const _last = dataM.lastPrice(symbol)
      const lastPrice = _last === 0 ? initialLast : _last

      // Maker LIMIT orders slightly worse than last price
      const oSellM = genLimitSell({ price: lastPrice * 1.003 })
      const oBuyM = genLimitBuy({ price: lastPrice * 1.002 })

      // Taker LIMIT orders, that should exec as MARKET
      const oSellT = genLimitSell({ price: lastPrice * 1.001 })
      const oBuyT = genLimitBuy({ price: lastPrice * 1.004 })
