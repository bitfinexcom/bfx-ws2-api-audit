...
module.exports = ({ symbol, amount, initialLast, dataDelay }) => {
  ...
  return {
    ...
    exec: ({ dataM, dataT, wsM, wsT, refreshSnapshots }) => {
      ...
      const oSellM = genLimitSell({ price: lastPrice * 1.03 })
      const oBuyM = genLimitBuy({ price: lastPrice * 1.02 })

      // Taker LIMIT orders, that should exec as MARKET
      const oSellT = genLimitSell({ price: lastPrice * 1.01 })
      const oBuyT = genLimitBuy({ price: lastPrice * 1.04 })
