...
module.exports = ({ symbol, amount, initialMid, dataDelay }) => {
  ...
  return {
    ...
    exec: ({ dataM, dataT, wsM, wsT, refreshSnapshots }) => {
      ...
      // Generates OCOs with targets at mid price
      ...
      const oBuyLimitT = genLimitBuy({ price: midPrice })
      const oSellLimitT = genLimitSell({ price: midPrice })

      // Orders to take OCO stop prices
      const oBuyStopT = genLimitBuy({ price: midPrice * 0.9 })
      ...
      return submitOrder(lastPriceSellM, wsM, dataM).then(() => {
        ...
        assertOrderRemoved({
          amountOrig: ocoSellM.amountOrig,
          price: ocoSellM.priceAuxLimit
        }, dataM)
