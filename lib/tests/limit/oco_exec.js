  ...
  return {
    ...
    exec: ({ dataM, dataT, wsM, wsT, refreshSnapshots }) => {
      ...
      return submitOrder(lastPriceSellM, wsM, dataM).then(() => {
        ...
        assert(dataT.lastPrice(symbol), lastPriceSellM.price)
      }).then(() => {
        ...
        assert(stopTriggerFillBuyT.priceAvg === dataT.lastPrice(symbol))
        assert(stop.priceAvg === dataM.lastPrice(symbol))
