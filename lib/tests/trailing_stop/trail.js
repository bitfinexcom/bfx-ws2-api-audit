...
module.exports = ({ symbol, amount, initialMid, dataDelay }) => {
  ...
  return {
    ...
    exec: ({ dataM, dataT, wsM, wsT, refreshSnapshots }) => {
      ...
      const oBuyLimitAM = genLimitBuy({ price: midPrice * 0.9985 })
      const oBuyLimitBM = genLimitBuy({ price: midPrice * 0.9990 })
      const oBuyLimitCM = genLimitBuy({ price: midPrice * 0.9995 })

      const oSellLimitAM = genLimitSell({ price: midPrice * 1.0015 })
      const oSellLimitBM = genLimitSell({ price: midPrice * 1.0010 })
      const oSellLimitCM = genLimitSell({ price: midPrice * 1.0005 })

      const delta = 100 // midPrice * 0.1
      ...
      return Promise.all([
        ...
        submitOrder(oSellLimitAM, wsM, dataM).then(() => {
          return submitOrder(oMarketBuyAT, wsT, dataT)
        })
      ]).then(() => {
        ...
        // bump top ask
        return Promise.all([
          dataT.awaitTicker(symbol, 30 * 1000),

          submitOrder(oSellLimitBM, wsM, dataM).then(() => {
            return submitOrder(oMarketBuyBT, wsT, dataT)
          })
        ])
      }).then(() => {
        ...
        return Promise.all([
          ...
          submitOrder(oSellLimitCM, wsM, dataM).then(() => {
            return submitOrder(oMarketBuyCT, wsT, dataT)
          })
        ])
      }).then(() => {
        ...
        return Promise.all([
          ...
          submitOrder(oBuyLimitBM, wsM, dataM).then(() => {
        ...
        return Promise.all([
          ...
          submitOrder(oBuyLimitCM, wsM, dataM).then(() => {
